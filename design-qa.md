...
| Image quality and assets | The five menu-bar themes are rendered programmatically at menu-bar resolution. The original Status Pulse app icon remains sharp at standard `.icns` sizes. |
| Copy and content | Language, five icon choices, elapsed-time visibility, and launch-at-login controls match the requested settings scope. |

## Interaction QA

- Language opens a nested System Language / 한국어 / English selector.
- Status Pulse, Progress Blocks, Signal Orbit, Window Check, and Layered S display live previews and persist the selected theme.
