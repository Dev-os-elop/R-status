...
- 메뉴 창의 상하좌우 바깥 여백을 14pt에서 7pt로 절반 축소
- 왼쪽 패널과 우측 메뉴 사이 간격도 14pt에서 7pt로 절반 축소
- 내부 패널과 카드 크기는 유지하면서 전체 메뉴를 430×440pt에서 409×426pt로 축소

## 0.14.25 - 2026-07-12

- 메뉴 윈도우와 루트 뷰에 중복 적용되던 70% 배경을 완전 투명으로 변경
- 왼쪽 패널과 우측 메뉴 카드에만 한 번 70% 불투명도를 적용해 실제 배경 투과 효과 구현
- 다크모드 패널과 타일도 88%에서 70% 불투명도로 통일

## 0.14.24 - 2026-07-12

- Run History의 ? 클릭 설명을 별도 경고창에서 제목 옆 작은 팝오버로 변경
- 팝오버 바깥을 클릭하면 자동으로 닫히고, ?를 다시 클릭해도 닫히도록 처리
- 보관 규칙 팝오버에 부드러운 열림 애니메이션 적용
...
- Run History 제목을 Appearance와 동일한 14pt semibold 및 secondary label 색상으로 통일
...
- 스위치 트랙의 회색/파란색 전환에도 동일한 애니메이션 적용
- 빠르게 연속 전환할 때 현재 애니메이션 위치에서 자연스럽게 이어지도록 presentation layer 기준 처리

## 0.14.17 - 2026-07-11

- 우측 메뉴 카드의 아이콘과 글자 묶음을 카드 내부 상하좌우 정중앙에 배치
- 전체 메뉴 높이를 470pt에서 440pt로 축소
- 왼쪽 콘텐츠 패널과 우측 메뉴 높이를 442pt에서 412pt로 축소
- Icon의 Status Preview 마지막 행 아래 안전 여백을 유지하면서 Main·Settings 레이아웃 재배치
...
- Main 화면 복귀 시 선택 하이라이트와 최신 상태·리소스·진행 정보 동시 갱신
...
- 우측 내비게이션 영역을 90pt, 각 카드 폭을 78pt로 축소
- 전체 고정 메뉴 폭을 482pt로 축소
- 모든 우측 카드에 마우스 hover 시 연한 파란색 하이라이트 추가
- Main 100% 진행 문자열과 Icon/Settings/History를 350pt 폭에서 재검증

## 0.14.3 - 2026-07-11

- Progress 8칸과 100% 표시가 들어가는 범위에서 좌측 콘텐츠 폭을 430pt에서 400pt로 축소
- 전체 고정 메뉴 폭을 586pt에서 556pt로 축소
- R processes와 Elapsed time 사이 간격을 약 38pt에서 18pt로 축소
- Icon/Settings/History 내부 레이아웃을 400pt 콘텐츠 폭에 맞춰 재배치

## 0.14.2 - 2026-07-11

- 고정 대시보드 높이를 546pt에서 470pt로 줄여 불필요한 여백 제거
- 우측 내비게이션을 전용 카드 컨트롤로 교체해 아이콘과 글자를 한 묶음으로 중앙 정렬
- Main의 상태·리소스·진행·버튼 영역을 촘촘하게 재배치
- Icon/Settings/History 화면을 줄어든 고정 높이에 맞춰 재정렬
- History 행 높이를 줄여 최대 5개 기록이 패널 안에 표시되도록 조정

## 0.14.1 - 2026-07-11

- Main 화면의 과도한 세로 여백을 줄이고 리소스·진행 정보를 압축 배치
- 우측 내비게이션을 5개의 독립된 둥근 사각형 카드로 복원
- Settings를 Basic/Language와 Advanced 옵션 구획으로 명확히 분리
- Progress와 Remaining을 리소스 항목과 동일한 파란색·크기·굵기로 통일
- 완료된 progress 블록을 고정 파란색으로 표시
...
- 각 기록에 Mac 로컬 지역·시간대 기준 완료 시각 표시
- 흰 카드 배경을 제거하고 구분선 목록으로 변경
- 기록 두 줄을 각 행 상하 중앙에 정렬
- Clear가 창을 닫지 않고 같은 패널에서 목록만 비우도록 수정
...
- Settings는 톱니바퀴 아이콘과 기존 들여쓰기 유지
- 두 액션 행의 단축키 표시와 클릭 취소 동작 유지
...
- CPU와 Memory를 각각 퍼센트 한 줄로 간결하게 표시

## 0.11.0 - 2026-07-11

- Memory 사용량 막대를 제거하고 `Memory: n%` 영문 한 줄로 단순화
- CPU 아래 왼쪽에 현재 사용률 막대 추가
- CPU 막대 오른쪽에 최근 약 1분의 사용량을 보여주는 실시간 라인 차트 추가
- CPU 기록을 2초 간격 최대 30개 샘플로 유지

## 0.10.2 - 2026-07-11

- Return to Ready 버튼 글자의 실제 렌더링 크기를 측정해 상하·좌우 정중앙 배치
- 한국어와 영어에서 동일한 중앙 정렬 방식 적용

## 0.10.1 - 2026-07-11

- RStudio Memory 사용률 아래에 전체 메모리 대비 파란 사용량 막대 추가
- `Memory: n%`와 파란 사용량 막대만 표시해 메뉴를 간결하게 유지
- 라이트·다크 모드에 맞춰 막대 배경색 자동 조정
