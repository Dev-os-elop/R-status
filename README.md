...
- 진행 패키지가 없어도 동일 코드를 한 번 이상 완료했다면 이전 기록 중앙값으로 예상 남은 시간 표시
- 상태 초기화
- 알림 테스트
- RStudio 열기
...
Addin으로 실행한 코드가 `progress::progress_bar` 또는 `progressr`를 사용하면 R Resource Usage 아래에 정확한 진행률과 남은 시간이 나타납니다.
...
`progress` 패키지는 `tick()`, `update()`, `terminate()`를 자동 감지합니다. `progressr`는 실행 중에만 전용 handler를 추가하고 종료 후 사용자의 기존 handler 설정을 복원합니다. 진행 이벤트는 최대 약 0.25초 간격으로 전달하므로 tick이 매우 잦아도 네트워크 전송이 연산을 과도하게 느리게 하지 않습니다.

진행 패키지가 없는 일반 코드는 Addin이 선택 코드의 MD5를 자동 생성합니다. 같은 코드가 이전에 정상 완료된 기록이 있으면 최근 기록의 실행시간 중앙값을 이용해 `Estimated remaining (history)`를 표시합니다. 첫 실행이나 코드가 변경된 경우에는 예측하지 않습니다. 코드를 별도 process에서 미리 실행하지 않으므로 연산 중복과 파일·DB 변경 같은 부작용이 발생하지 않습니다. History를 Clear하면 이 예측 기록도 함께 삭제됩니다.
