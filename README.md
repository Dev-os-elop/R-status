...
소스에서 직접 빌드할 때만 Xcode 15 이상 또는 호환되는 Xcode Command Line Tools가 필요합니다.
...
앱의 업데이트 확인 기능은 `Dev-os-elop/R-status`의 `main` 브랜치에 있는 `Resources/Info.plist` 버전을 기준으로 합니다. 새 버전을 배포할 때는 앱과 R 패키지 버전을 맞추고 `main`에 반영한 뒤, 같은 버전 태그의 GitHub Release에 `RStudioStatus-macos-arm64` 실행 파일을 첨부하세요. `Resources/RStudioStatus-macos-arm64.sha256`에는 해당 파일의 SHA-256을 기록합니다.
