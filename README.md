...
- 실행 경과 시간 표시
- R 관련 CPU·프로세스·병렬 worker·thread 실시간 표시
- 완료·실패 시 macOS 알림
...
- **Active**: 샘플 시점에 실제로 CPU를 사용 중인 R 프로세스 수입니다.
- **Parallel workers**: `parallel`, PSOCK 등으로 생성된 병렬 작업용 R 프로세스 수입니다.
- **R processes**: main R 세션과 병렬 worker를 합친 전체 R 관련 프로세스 수입니다.
- **OS threads**: 모든 R 관련 프로세스 내부의 운영체제 thread 합계입니다. BLAS 같은 내부 병렬화는 process가 늘지 않고 thread만 늘어날 수 있습니다.

PSOCK cluster처럼 worker의 부모 PID가 분리되는 경우에는 실행 인자에서 `workRSOCK`과 `MASTER` 정보를 함께 확인해 worker를 계산합니다.

병렬 지표를 확인하려면 [`examples/parallel-resource-test-20-seconds.R`](examples/parallel-resource-test-20-seconds.R)을 전체 선택하고 **Addins → Run Selection with Status**로 실행하세요. worker 3개가 약 20초 동안 행렬 연산을 수행하므로 일반적으로 `Active: 3`, `Parallel workers: 3`, `R processes: 4`와 약 300%에 가까운 CPU가 표시됩니다. main R 세션 1개는 worker 결과를 기다리므로 active에는 포함되지 않습니다. 실제 수치는 시스템과 샘플 시점에 따라 달라집니다.
