...
Addin으로 실행한 코드가 `progress::progress_bar` 또는 `progressr`를 사용하면 R Resource Usage 아래에 정확한 진행률과 남은 시간이 나타납니다. 진행 패키지를 사용하지 않은 선택 코드도 첫 번째 `for` 루프의 반복 횟수를 Addin이 자동 계측합니다.
...
자동 계측은 사용자가 선택한 코드에 명시적인 progress API가 없을 때 첫 번째 `for` 루프를 내부적으로 progress tick으로 감싸는 방식입니다. 반복 코드 자체는 한 번만 실행되며, 각 반복이 끝날 때 진행률과 ETA가 갱신됩니다. `while` 루프나 반복 횟수를 미리 알 수 없는 임의 연산은 자동 ETA를 계산하지 않습니다.
